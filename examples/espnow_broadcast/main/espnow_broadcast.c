...
static void recv_cb(const esp_now_recv_info_t *recv_info, const uint8_t *data, int len) {
    ...
    ESP_LOGI(TAG, "RX from " MACSTR " rssi=%d: %.*s", MAC2STR(src), recv_info->rx_ctrl->rssi, len, (const char *)data);
}

static void send_cb(const esp_now_send_info_t *tx_info, esp_now_send_status_t status) {
    ESP_LOGD(TAG, "TX to " MACSTR ": %s", MAC2STR(tx_info->des_addr), status == ESP_NOW_SEND_SUCCESS ? "OK" : "FAIL");
}
