...
static void _espnow_recv_cb(const esp_now_recv_info_t *recv_info,
...
static void _espnow_send_cb(const esp_now_send_info_t *tx_info,
                             esp_now_send_status_t status)
{
    if (status != ESP_NOW_SEND_SUCCESS) {
        s_tx_failed++;
        ESP_LOGD(TAG, "TX fail to " MACSTR " (total=%"PRIu32")",
                 MAC2STR(tx_info->des_addr), s_tx_failed);
    }
}
...
static esp_err_t _espnow_init(void)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "esp_now_init failed: %s  (is Wi-Fi started?)",
                 esp_err_to_name(err));
    ...
    s_initialized = true;
    uint8_t mac[6]; uint8_t ch; wifi_second_chan_t sc;
 ...
 */
z_result_t _z_open_udp_multicast(_z_sys_net_socket_t *sock,
                                  const _z_sys_net_endpoint_t rep,
                                  _z_sys_net_endpoint_t *lep,
                                  uint32_t tout, const char *iface)
{
    (void)rep; (void)tout; (void)iface;
 ...
 */
z_result_t _z_listen_udp_multicast(_z_sys_net_socket_t *sock,
                                    const _z_sys_net_endpoint_t rep,
                                    uint32_t tout, const char *iface,
                                    const char *join)
{
    (void)rep; (void)iface; (void)join;
    if (_espnow_init() != ESP_OK) return _Z_ERR_GENERIC;
...
void _z_close_udp_multicast(_z_sys_net_socket_t *sockrecv,
                             _z_sys_net_socket_t *socksend,
                             const _z_sys_net_endpoint_t rep,
                             const _z_sys_net_endpoint_t lep)
{
    (void)socksend; (void)rep; (void)lep;
    ...
    s_initialized = false;
    sockrecv->_fd = -1;
    ESP_LOGI(TAG, "Closed  rx_dropped=%"PRIu32"  tx_failed=%"PRIu32,
             s_rx_dropped, s_tx_failed);
}
 ...
 */
size_t _z_read_udp_multicast(const _z_sys_net_socket_t sock,
 ...
 */
size_t _z_read_exact_udp_multicast(const _z_sys_net_socket_t sock,
                                    uint8_t *ptr, size_t len,
                                    const _z_sys_net_endpoint_t lep,
                                    _z_slice_t *addr)
{
 ...
 */
size_t _z_send_udp_multicast(const _z_sys_net_socket_t sock,
                              ...
                              const _z_sys_net_endpoint_t rep)
{
    ...
    s_tx_failed++;
    ESP_LOGW(TAG, "TX failed: %s (total=%"PRIu32")",
             esp_err_to_name(err), s_tx_failed);
    return SIZE_MAX;
}
